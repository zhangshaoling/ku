...
int main(int argc,char**argv){
    ...
    const Case equality_cases[]={{"thought string_equal() { \"ku\" == \"ku\" }","string_equal",1,DAO_VALUE_TRIT},{"thought null_equal() { null == null }","null_equal",1,DAO_VALUE_TRIT},{"thought trit_equal() { true == true }","trit_equal",1,DAO_VALUE_TRIT}};
    const Case ku_v1_cases[]={
        ...
        {"thought field_access() { user = {\"name\": \"dao\", \"age\": 42}; user.age }","field_access",42},
        {"thought map_for() { values = {\"z\": 1, \"a\": 2}; total = 0; for key, value in values { total = total + value } total }","map_for",3},
    ...
    const auto run_cases=[&](dao_module*compiler_module,dao_function compile_function){
      ...
      return run_group(cases,std::size(cases))&&run_group(logic_cases,std::size(logic_cases))&&run_group(equality_cases,std::size(equality_cases))&&run_group(ku_v1_cases,std::size(ku_v1_cases));
    };
