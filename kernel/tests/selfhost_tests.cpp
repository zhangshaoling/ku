...
int main(int argc,char**argv){
    ...
    struct Case{const char*source;const char*name;int64_t expected;uint8_t type=DAO_VALUE_I64;};const Case cases[]={{"thought main() { 40 + 2 }","main",42},{"thought product() { 6 * 7 }","product",42},{"thought difference() { 50 - 8 }","difference",42},{"thought quotient() { 84 / 2 }","quotient",42},{"thought precedence() { 2 + 3 * 4 }","precedence",14},{"thought mixed() { 20 / 2 + 32 }","mixed",42},{"thought remainder() { 86 % 44 }","remainder",42},{"thought chain() { 100 - 8 * 7 - 2 }","chain",42},{"thought parens() { (2 + 3) * 8 + 2 }","parens",42},{"thought unary() { 50 + -8 }","unary",42},{"thought nested() { 2 * (10 + 11) }","nested",42},{"thought first() { 40 + 2 } thought second() { 6 * 7 }","first",42},{"thought first() { 40 + 2 } thought second() { 6 * 7 }","second",42},{"thought with_args(left, right) { left + right }","with_args",42},{"thought local() { value = 40; value + 2 }","local",42},{"thought reassigned() { value = 40; value = value + 2; value }","reassigned",42},{"thought double(value) { value * 2 } thought called() { double(21) }","called",42},{"thought add(left, right) { left + right } thought called() { add(20, 22) }","called",42},{"thought answer() { 42 } thought called() { answer() }","called",42},{"thought add(left, right) { left + right } thought with_args(left, right) { add(left, right) }","with_args",42},{"thought add(left, right) { left + right } thought calculated() { add(8 * 2, 30 - 4) }","calculated",42},{"thought equal() { 21 == 21 }","equal",1,DAO_VALUE_TRIT},{"thought unequal() { 21 != 22 }","unequal",1,DAO_VALUE_TRIT},{"thought ordered() { 20 < 22 }","ordered",1,DAO_VALUE_TRIT},{"thought ordered() { 22 <= 22 }","ordered",1,DAO_VALUE_TRIT},{"thought ordered() { 22 > 20 }","ordered",1,DAO_VALUE_TRIT},{"thought ordered() { 22 >= 22 }","ordered",1,DAO_VALUE_TRIT},{"thought false_ordered() { 22 < 20 }","false_ordered",-1,DAO_VALUE_TRIT}};
    for(const auto&test:cases){const std::string source=test.source;dao_value arg{};if(dao_value_make_string_view({reinterpret_cast<const uint8_t*>(source.data()),source.size()},&arg)!=DAO_OK)return EXIT_FAILURE;dao_value generated{};if(dao_vm_call(vm,module,function,&arg,1,&generated,&error)!=DAO_OK){std::cerr<<error.message<<'\n';return EXIT_FAILURE;}dao_bytes output{};if(dao_value_get_view(&generated,&output)!=DAO_OK)return EXIT_FAILURE;dao_module*result_module=nullptr;if(dao_vm_load_module(vm,output,&result_module,&error)!=DAO_OK)return EXIT_FAILURE;dao_function generated_fn=0;if(dao_module_find_export(result_module,symbol(test.name),&generated_fn)!=DAO_OK)return EXIT_FAILURE;const dao_value supplied[]={{DAO_VALUE_I64,0,40},{DAO_VALUE_I64,0,2}};const bool has_args=std::strcmp(test.name,"with_args")==0;dao_value result{};const bool ok=dao_vm_call(vm,result_module,generated_fn,has_args?supplied:nullptr,has_args?2:0,&result,&error)==DAO_OK&&result.type==test.type&&result.payload==test.expected;dao_module_release(result_module);if(!ok)return EXIT_FAILURE;}
