...
_KW_CN = {
    ...
    '真': 'true', '假': 'false', '空': 'null',
    '且': 'and', '或': 'or', '非': 'not',
    '引': 'import', '别': 'as',
}
def _kw(val):
    return _KW_CN.get(val, val)
...
def _stmt(tokens, pos, end):
    ...
    # thought definition
    if t["type"] == "keyword" and _kw(t["value"]) == "thought":
    ...
    # for
    if t["type"] == "keyword" and _kw(t["value"]) == "for":
        return _parse_for(tokens, pos + 1, end)

    # return
    if t["type"] == "keyword" and _kw(t["value"]) == "return":
        return _parse_return(tokens, pos + 1, end)

    # break / continue
    if t["type"] == "keyword" and _kw(t["value"]) == "break":
        return _node("break", "", []), pos + 1
    if t["type"] == "keyword" and _kw(t["value"]) == "continue":
        return _node("continue", "", []), pos + 1

    # try/catch
    if t["type"] == "keyword" and _kw(t["value"]) == "try":
        return _parse_try(tokens, pos + 1, end)

    # throw
    if t["type"] == "keyword" and _kw(t["value"]) == "throw":
        val, new_pos = _expr(tokens, pos + 1, end)
        return _node("throw", "", [val]), new_pos

    # import: 引 "path" 别 alias
    if t["type"] == "keyword" and _kw(t["value"]) == "import":
        # expect: 引 "path" 别 alias
        path_tok = _peek(tokens, pos + 1)
        alias_tok = _peek(tokens, pos + 3)  # skip 引, path, 别
        path_val = path_tok["value"] if path_tok["type"] == "string" else ""
        alias_val = alias_tok["value"] if alias_tok["type"] == "name" else ""
        new_pos = pos + 4
        # skip optional trailing tokens
        while new_pos < end and tokens[new_pos]["type"] in ("newline", "eof"):
            new_pos += 1
        return _node("import", alias_val, [_lit(path_val)]), new_pos
...
def _parse_if(tokens, pos, end):
    ...
    # explicit else/否 may start on the next line
    else_pos = pos
    while else_pos < end and _peek(tokens, else_pos)["type"] == "newline":
        else_pos += 1
    t = _peek(tokens, else_pos)
    if t["type"] == "keyword" and _kw(t["value"]) == "else":
...
def _parse_try(tokens, pos, end):
    ...
    # optional catch
    if t["type"] == "keyword" and _kw(t["value"]) == "catch":
        pos += 1
        if _peek(tokens, pos)["value"] == "(":
            pos += 1
    ...
    # optional finally
    if t["type"] == "keyword" and _kw(t["value"]) == "finally":
        pos += 1
        finally_body, pos = _parse_brace(tokens, pos, end)

    return _node("try", "", [try_body, _ref(catch_var) if catch_var else _lit(""), catch_body or _lit(""), finally_body or _lit("")]), pos
...
def _prefix(tokens, pos, end):
    ...
    if t["type"] == "keyword":
        ...
        if kw_val == "null":
            return _lit(""), pos + 1
        # and/or/not used as function calls: and(a, b) → op(and, a, b)
        if kw_val in ("and", "or") and pos + 1 < end:
            next_t = _peek(tokens, pos + 1)
            if next_t["value"] == "(":
                args, new_pos = _parse_call_args(tokens, pos + 1, end)
                if len(args) == 2:
                    return _op(kw_val, args[0], args[1]), new_pos
        if kw_val == "not" and pos + 1 < end:
            next_t = _peek(tokens, pos + 1)
            if next_t["value"] == "(":
                args, new_pos = _parse_call_args(tokens, pos + 1, end)
                if len(args) == 1:
                    return _op("not", args[0], _lit("")), new_pos

        # if expression: if (cond) { then } { else }
        if kw_val == "if":
            return _parse_if_expr(tokens, pos + 1, end)

        # try expression: try { ... } catch var { ... } finally { ... }
        if kw_val == "try":
            return _parse_try(tokens, pos + 1, end)
    ...
    if t["type"] == "op" and t["value"] in ("+", "-", "*", "/", "%") and pos + 1 < end:
        next_t = _peek(tokens, pos + 1)
        if next_t["value"] == "(":
            args, new_pos = _parse_call_args(tokens, pos + 1, end)
            return _call(t["value"], args), new_pos
    # comparison operators as prefix: >= (a, b) → op(>=, a, b)
    if t["type"] == "op" and t["value"] in ("==", "!=", "<", ">", "<=", ">=") and pos + 1 < end:
        next_t = _peek(tokens, pos + 1)
        if next_t["value"] == "(":
            args, new_pos = _parse_call_args(tokens, pos + 1, end)
            if len(args) == 2:
                return _op(t["value"], args[0], args[1]), new_pos
...
def _parse_call_args(tokens, pos, end):
    ...
    while pos < end and depth > 0:
        ...
        if t["type"] == "punct":
            ...
            if t["value"] == "}":
                brace_depth -= 1
            if t["value"] == "," and depth == 1 and bracket_depth == 0 and brace_depth == 0:
                arg, _ = _expr(tokens, arg_start, pos)
                args.append(arg)
                arg_start = pos + 1
        pos += 1
...
def _parse_dict(tokens, pos, end):
    ...
    while pos < end and depth > 0:
        ...
        if t["value"] == ":" and depth == 1 and state == "key":
            ...
            key_start = pos + 1
            state = "val"
        if t["value"] == "," and depth == 1 and state == "val":
            val, _ = _expr(tokens, key_start, pos)
            pairs.append(_node("pair", cur_key, [val]))
            key_start = pos + 1
            state = "key"
