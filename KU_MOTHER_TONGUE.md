...
本文是 Ku 的中文入口说明。旧版本把早期构想、语法草稿和乱码样例混在一起，容易让人误以为项目目标是“做一门普通中文脚本语言”。当前项目目标更窄，也更硬：
...
- **Ku**：语言系统、产品、语法和生态的唯一名称。
- **Dao**：现有 `dao_*` ABI、Dao Binary Module、`dao/` 旧实现和历史文档中的内部兼容名称。

以后写产品、架构、语法和项目路线时默认说 **Ku**。只有精确引用现有内部符号、格式或目录时才使用 Dao，并说明它是实现标识。完整规则见 `docs/KU_NAMING_AND_AUTHORITY.md`。
...
Ku 的“中文”不只是关键字翻译。中文表层要服务于记忆和思维结构：
...
这些方向可以回到路线图里，但当前项目边界和完成标准以 `docs/KU_SUBPROJECT_WORKSHEETS.md` 为准。
...
5. 在 Ku 新内核和自举编译器足够强以后，再把更多 bootstrap 生成逻辑移出 Python。
