"""
ku compiler — 栈式虚拟机字节码编译器 v0.1
==========================================
将 ku AST 编译成字节码，由 KuVM 执行。
...
# 复用 runtime 的解析器
try:
    from .runtime import parse_ku, _parse_body, _parse_expr, Node, Thought, KuEnv, _inject_builtins, _simplify
...
class KuVM:
    ...
    def __init__(self, env: Optional[KuEnv] = None):
        self.env = env or KuEnv()
...
def compile_ku(source: str) -> dict:
    """编译 ku 源码为字节码。"""
    return KuCompiler().compile(source)


def compile_ku_file(path: str) -> dict:
    """编译 ku 文件为字节码。"""
    return KuCompiler().compile_file(path)


def run_bytecode(bytecode: dict, env: Optional[KuEnv] = None) -> Any:
    """执行字节码。"""
    vm = KuVM(env)
    return vm.execute(bytecode)


def run_ku_compiled(source: str) -> Any:
    """编译并执行 ku 源码。"""
