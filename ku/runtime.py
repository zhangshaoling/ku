...
KU_HOME = Path(os.environ.get("KU_HOME", Path(__file__).parent))
KU_DATA_DIR = Path(os.environ.get("KU_DATA_DIR", KU_HOME / "data"))
    ...
    def call(self, args: Optional[list] = None, env: Optional[dict] = None):
        ...
        if callable(self.body):
            try:
                ...
                result = self.body() if args is None else self.body(*args)
...
def parse_ku(source: str) -> list[Thought]:
    """
    ...
    """
    from .ku_lexer import lex as ku_lex
    from .ku_parser import parse_tokens_as_nodes
    ...
    while pos < total:
        ...
        if tok["type"] != "keyword" or tok["value"] != "thought":
            continue
...
_import_re = re.compile(r'^import\s+"([^"]+)"(?:\s+as\s+(\w+))?\s*$')
_import_re_single = re.compile(r"^import\s+'([^']+)'(?:\s+as\s+(\w+))?\s*$")


def _parse_imports(source: str) -> list[tuple[str, Optional[str]]]:
    """提取源码中的 import 指令。

    返回 [(path, prefix_or_None), ...]
    支持: import "path"   import "path" as prefix
    """
    imports = []
...
def _resolve_import_path(import_path: str, current_dir: Optional[str] = None) -> Optional[Path]:
    ...
    if import_path.startswith("."):
        ...
        # 在 KU_HOME / std 下查找
        candidates.append(KU_HOME / p_path)
        candidates.append(KU_HOME / p_path.with_suffix(".ku"))
        # 也查找原始路径（如果是完整路径）
...
class KuEnv:
    ...
    def _import_module(self, import_path: str, prefix: Optional[str] = None,
        ...
        if resolved is None:
            raise FileNotFoundError(f"ku: 找不到模块 '{import_path}' (搜索路径: {current_dir or 'KU_HOME'})")
        ...
        thoughts = parse_ku(clean_source)
    ...
    def save_state(self, path: Optional[str] = None):
        """将所有 thought 持久化到磁盘。"""
        save_path = Path(path or str(KU_DATA_DIR / ".ku_state.json"))
...
def _inject_builtins(env: KuEnv):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="ku — 玄璃母语运行时")
    ...
    # 自动加载 base.ku
    base_path = KU_HOME / "base.ku"
    if base_path.exists():
        env.load(str(base_path))

    # 自动加载标准库 std/*.ku
    std_path = KU_HOME / "std"
    if std_path.exists():
...
try:
    from .ku_parser import parse_tokens as _py_parse_tokens, parse_tokens_as_nodes as _py_parse_tokens_nodes, dict_to_node as _py_dict_to_node
